...
void setup()
{
  ...
  TCCR2A |= (1 << WGM21); // CTC mode. clear counter on TCNT2 == OCR2A
  OCR2A = 10; // Interrupt every 25600th cpu cycle (256*100)
  TCNT2 = 0x00; // start counting at 0
  TCCR2B |= (1 << CS22) | (0x01 << CS21); // Start the clock with a 256 prescaler
...
ISR (TIMER2_COMPA_vect)
{
  ...
  for (int i=0; i<8; i++)
  {
    SPI.transfer(cube[current_layer][i]);
  }
  SPI.endTransaction();
  
  PORTD = (current_layer << 5) | (PORTD & B00011111);  
  current_layer++;
